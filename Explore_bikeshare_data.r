...
summary(ny)
summary(wash)
chSum = signif(sum(chi$Trip.Duration,na.rm=TRUE) / 3600,digits=3)
nySum = signif(sum(ny$Trip.Duration,na.rm=TRUE) / 3600,digits=3)
whSum = signif(sum(wash$Trip.Duration,na.rm=TRUE) / 3600,digits=3)
...
chMean = signif(mean(chi$Trip.Duration,na.rm=TRUE),digits=3)
nyMean = signif(mean(ny$Trip.Duration,na.rm=TRUE),digits=3)
whMean = signif(mean(wash$Trip.Duration,na.rm=TRUE),digits=3)
